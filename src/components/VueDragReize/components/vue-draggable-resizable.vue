...
<!-- 编辑型拖拽组件 -->
  <div :style="style" :class="[
    ...
    }, className,showGuide?'drag-wrap':'normal-drag-wrap']" @mousedown="elementMouseDown" @touchstart="elementTouchDown">
    <!-- 控制手柄 -->
    <div v-for="handle in actualHandles" :key="handle.index" :class="[classNameHandle, classNameHandle + '-' + handle]"
      :style="handleStyle(handle)" @mousedown.stop.prevent="handleDown(handle, $event)"
    ...
    methods: {
      ...
      elementDown(e) {
        ...
        if (this.$el.contains(target)) {
          ...
          if (!this.enabled) {
            ...
            // 传递索引到辅助线
            this.showGuide=true;
            this.$emit('activated', this.dragIndex)
      ...
      calcDragLimits() {
        return {
          ...
          maxBottom: Math.floor((this.parentHeight - this.height - this.bottom) / this.grid[1]) * this.grid[1] + this
            .bottom
        }
      },
      // 取消
      deselect(e) {
        ...
        if (!this.$el.contains(target) && !regex.test(target.className)) {
          if (this.enabled && !this.preventDeactivation) {
            ...
            this.$emit('deactivated',this.dragIndex)
      ...
      async handleDrag(e) {
        ...
        const tmpDeltaX = axis && axis !== 'y' ? mouseClickPosition.mouseX - (e.touches ? e.touches[0].pageX : e
          .pageX) : 0
        const tmpDeltaY = axis && axis !== 'x' ? mouseClickPosition.mouseY - (e.touches ? e.touches[0].pageY : e
          .pageY) : 0
      ...
      handleResize(e) {
        ...
        this.width = width
        this.height = height
        this.$emit('resizing', this.left, this.top, this.width, this.height, this.dragIndex)
      },
      changeWidth(val) {
        ...
        if (this.isConflictCheck) {
          ...
          for (let item of nodes) {
            if (item.className !== undefined && !item.className.includes(this.classNameActive) && item.getAttribute(
              ...
              // 左上角与右下角重叠
              const tfAndBr = (top >= tt && left >= tl && tt + th > top && tl + tw > left) || (top <= tt && left < tl &&
                top + height > tt && left + width > tl)
              // 右上角与左下角重叠
              const brAndTf = (left <= tl && top >= tt && left + width > tl && top < tt + th) || (top < tt && left >
                tl && top + height > tt && left < tl + tw)
              // 下边与上边重叠
              const bAndT = (top <= tt && left >= tl && top + height > tt && left < tl + tw) || (top >= tt && left <=
                tl && top < tt + th && left > tl + tw)
              // 上边与下边重叠（宽度不一样）
              const tAndB = (top <= tt && left >= tl && top + height > tt && left < tl + tw) || (top >= tt && left <=
                tl && top < tt + th && left > tl + tw)
              // 左边与右边重叠
              const lAndR = (left >= tl && top >= tt && left < tl + tw && top < tt + th) || (top > tt && left <= tl &&
                left + width > tl && top < tt + th)
              // 左边与右边重叠（高度不一样）
              const rAndL = (top <= tt && left >= tl && top + height > tt && left < tl + tw) || (top >= tt && left <=
                tl && top < tt + th && left + width > tl)
