  ...
  <div :style="wrapperStyle" class="vue-ruler-wrapper" onselectstart="return false;">
    <section v-show="rulerToggle">
      <div ref="horizontalRuler" class="vue-ruler-h" @mousedown.stop="horizontalDragRuler">
        <span v-for="(item,index) in xScale" :key="index" :style="{left:index * 100 + 2 + 'px'}"
          class="n">{{ item.id }}</span>
      </div>
      <div ref="verticalRuler" class="vue-ruler-v" @mousedown.stop="verticalDragRuler">
        <span v-for="(item,index) in yScale" :key="index" :style="{top:index * 100 + 2 + 'px'}"
          class="n">{{ item.id }}</span>
      </div>
      ...
      <div v-for="item in lineList" :title="item.title" :style="getLineStyle(item)" :key="item.id"
        :class="`vue-ruler-ref-line-${item.type}`" @mousedown="handleDragLine(item)"></div>
    </section>
  ...
  export default {
    ...
    props: {
      ...
      value: {
        type: Array,
        default: () => {
          return [] // { type: 'h', site: 100 }, { type: 'v', site: 180 }
        }
      }, // 预置参考线
      contentLayout: {
        type: Object,
      ...
      lineList() {
        ...
        return this.value.map((item) => {
          ...
          return {
            ...
            [isH ? 'top' : 'left']: item.site / (this.stepLength / 100) + this.size
          }
        })
      }
    },
    ...
    methods: {
      ...
      box() {
        ...
        if (this.parent) {
          ...
          // this.windowHeight = document.documentElement.clientHeight - this.topSpacing
            this.windowWidth = 2400 - this.leftSpacing;
            this.windowHeight = 2400 - this.topSpacing
        }
        // this.rulerWidth = this.$refs.verticalRuler.clientWidth
        // this.rulerHeight = this.$refs.horizontalRuler.clientHeight
        this.rulerWidth = 2400;
        this.rulerHeight = 2400
        this.setSpacing()
      }, // 获取窗口宽与高
      setSpacing() {
      ...
      getCalc(array, length) {
        for (let i = 0; i < length * this.stepLength / 100; i += this.stepLength) {
        ...
        if (this.isDrag) {
          ...
          switch (this.dragFlag) {
            case 'x':
              cloneList.push({
                type: 'h',
                site: ($event.pageY - this.topSpacing - this.size) * (this.stepLength / 100)
              })
            ...
            case 'y':
              cloneList.push({
                type: 'v',
                site: ($event.pageX - this.leftSpacing - this.size) * (this.stepLength / 100)
              })
              this.$emit('input', cloneList)
              break
            case 'h':
      ...
      dragCalc(list, page, spacing, ruler, type) {
        if (page - spacing < ruler) {
          ...
          list.splice(Index, 1, {
            type: type,
            site: (page - spacing - this.size) * (this.stepLength / 100)
          })
        }
      },
      horizontalDragRuler() {
...
<style lang="scss">
  .vue-ruler {
    ...
    &-h {
      ...
      left: 18px;
      opacity: 0.6;
      background: url('../components/image/ruler_h.png') repeat-x;
      /*./image/ruler_h.png*/
    }
    ...
    &-v {
      ...
      top: 18px;
      opacity: 0.6;
      background: url('../components/image/ruler_v.png') repeat-y;
      /*./image/ruler_v.png*/
    }
