  ...
  <div class="role-warp">
      ...
      <el-table v-loading="loading" :data="roleData">
        ...
        <el-table-column label="界面集成">
          <template slot-scope="scope">
            ...
            <br>
            <el-button type="text" icon="el-icon-data-analysis" @click="setScreen(scope.row)">分配大屏</el-button>
            <br>
            <el-button type="text" icon="el-icon-data-analysis" @click="setRes(scope.row)">分配应用</el-button>
    ...
    <el-dialog title="分配大屏" :visible.sync="setScreenDialog" width="33%">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="setRoleScreen">确 定</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    getRoleScreen() {
      return new Promise((resolve, reject) => {
        ...
        getRoleScreen(msg).then((res) => {
          ...
          this.screenForm.check = respData.check
          this.screenForm.options = new_screen
          console.log(res)
          resolve()
        }).catch(error => {
          reject(error)
    ...
    // 分配
    setScreen(row) {
      this.screenForm.roleId = row.id

      this.setScreenDialog = true
      // 查询之前配置角色大屏
      this.getRoleScreen()
    },
    // 分配用户拥有大屏
    setRoleScreen() {
      const msg = JSON.stringify({
        'roleId': this.screenForm.roleId,
        'check': this.screenForm.check
      })
      return new Promise((resolve, reject) => {
        setRoleScreen(msg).then((res) => {
          this.setScreenDialog = false
